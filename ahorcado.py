...
IMAGENES_AHORCADO = ['''
    ...
    =========''', '''
     ...
     / \  |
          |
    =========''']
palabras = 'hormiga babuino tejon murcielago oso castor camello gato almeja cobra pantera coyote cuervo ciervo perro burro pato aguila huron zorro rana cabra ganso halcon leon lagarto llama topo mono alce raton mula salamandra nutria buho panda loro paloma piton conejo carnero rata cuervo rinceronte salmon foca tiburon oveja mofeta perezoso serpiente araña cisne tigre sopa trucha pavo tortuga comadreja ballena lobo wombat cebra'.split()
...
def mostrarTablero(IMAGENES_AHORCADO, letrasIncorrectas, letrasCorrectas, palabraSecreta):
        ...
        for i in range(len(palabraSecreta)): # completar los espacios vacios con las letrras adivinadas
...
def obtenerIntento(letrasProbadas):
    ...
    while True:
        ...
        if len(intento) != 1:
            print('Por favor, itroduce una letra.')
...
while True:
    ...
    if intento in palabraSecreta:
        ...
        if len(letrasIncorrectas) == len(IMAGENES_AHORCADO) - 1:
            ...
            juegoTerminado = True
    if juegoTerminado:
        print('Gracias por jugar el AHORCADO de Ger (^◡^ )')
